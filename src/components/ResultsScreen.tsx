...
export default function ResultsScreen({ questions, answers, onRestart, onRetry }: Props) {
  ...
  const grade =
    pct >= 90 ? { label: t.excellent, color: "text-green-600", bg: "bg-green-50 border-green-200" } :
    pct >= 70 ? { label: t.goodJob, color: "text-blue-600", bg: "bg-blue-50 border-blue-200" } :
    pct >= 50 ? { label: t.keepStudying, color: "text-yellow-600", bg: "bg-yellow-50 border-yellow-200" } :
    { label: t.needsImprovement, color: "text-red-600", bg: "bg-red-50 border-red-200" };
    ...
    <div className="min-h-screen bg-gray-50 p-4 pb-16">
      <div className="max-w-xl mx-auto space-y-4">
        ...
        <div className={`rounded-2xl border p-6 text-center space-y-1 ${grade.bg}`}>
          ...
          <p className="text-gray-500 text-sm">{t.correctOf(score, questions.length)}</p>
        </div>
          ...
          return (
            <div key={q.id} className="bg-white rounded-2xl shadow-sm border border-gray-200 p-5 space-y-4">
              ...
              <div className="flex items-start gap-3">
                ...
                <p dir="auto" className="text-sm font-semibold text-gray-900 leading-snug">
                  {i + 1}. {q.question}
                </p>
              </div>
